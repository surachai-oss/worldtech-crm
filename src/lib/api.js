...
export const PAYMENT_METHOD_TRANSFER = 'โอนเงิน'
...
// catalogId = null คือ "ชุดกลาง" ที่ใช้กับทุกแคตตาล็อกที่ไม่ได้ตั้งปุ่มเอง
export const CATALOG_BUTTON_KINDS = [
  { value: 'link',  label: 'ลิงก์ (เว็บ / LINE / Facebook)', placeholder: 'https://line.me/ti/p/~worldtech' },
  { value: 'phone', label: 'เบอร์โทร',                       placeholder: '021234567' },
  { value: 'email', label: 'อีเมล',                          placeholder: 'sales@worldtech.asia' },
  { value: 'image', label: 'รูป (เช่น QR code)',              placeholder: 'ลิงก์ตอนกดรูป (ไม่ใส่ก็ได้)' },
]

export function listCatalogButtons(catalogId) {
  const q = supabase.from('catalog_buttons').select('*')
  // ชุดกลางคือแถวที่ catalog_id เป็น null ต้องใช้ .is() ไม่ใช่ .eq() — eq กับ null ไม่เคยแมตช์อะไรใน SQL
  const scoped = catalogId ? q.eq('catalog_id', catalogId) : q.is('catalog_id', null)
  return scoped.order('display_order', { ascending: true }).then(handle)
}

export async function addCatalogButton(catalogId, patch) {
  const rows = await supabase.from('catalog_buttons')
    .insert({ catalog_id: catalogId ?? null, ...patch }).select().then(handle)
  return rows[0]
}

export const updateCatalogButton = (id, patch) =>
  supabase.from('catalog_buttons').update(patch).eq('id', id).select().then(handle).then(r => r[0])

export async function deleteCatalogButton(button) {
  await supabase.from('catalog_buttons').delete().eq('id', button.id).then(handle)
  // ลบรูปที่ผูกกับปุ่มด้วย ไม่งั้นเหลือไฟล์กำพร้าใน bucket ที่ไม่มีอะไรอ้างถึงอีก
  if (button.image_path) {
    await supabase.storage.from(CATALOG_IMAGES_BUCKET).remove([button.image_path]).then(() => {}, () => {})
  }
}

export const reorderCatalogButtons = (orderedIds) =>
  supabase.rpc('catalog_reorder_buttons', { p_ids: orderedIds }).then(handle)

// รูปของปุ่ม (QR code ฯลฯ) เก็บใน bucket เดียวกับรูปแคตตาล็อก แต่แยกโฟลเดอร์ให้รู้ว่าเป็นคนละของ
export async function uploadCatalogButtonImage(file) {
  if (file.size > MAX_CATALOG_IMAGE_SIZE) throw new Error('ไฟล์ใหญ่เกิน 10MB')
  const safeName = file.name.replace(/[^\w.-]/g, '_').slice(-80)
  const path = `buttons/${Date.now()}-${safeName}`
  const { error } = await supabase.storage.from(CATALOG_IMAGES_BUCKET).upload(path, file, { contentType: file.type })
  if (error) throw error
  return { image_path: path, image_url: supabase.storage.from(CATALOG_IMAGES_BUCKET).getPublicUrl(path).data.publicUrl }
}

// แคตตาล็อกนี้ใช้ปุ่มชุดไหน — ของตัวเองถ้ามี ไม่มีก็ชุดกลาง
// ตรรกะเดียวกับที่ Netlify function ใช้ตอนส่งให้หน้าลูกค้า จะได้พรีวิวตรงกับของจริง
export async function resolveCatalogButtons(catalogId) {
  const own = await listCatalogButtons(catalogId)
  if (own.length) return { buttons: own, usingShared: false }
  return { buttons: await listCatalogButtons(null), usingShared: true }
}
