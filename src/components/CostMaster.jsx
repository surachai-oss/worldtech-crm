...
const COLUMN_HELP = {
  ...
  normal: {
    ...
    kind: 'กรอกเอง — จำเป็น',
    body: [
      ['คืออะไร', 'ราคาป้ายก่อนลด ใช้เป็นฐานคิด % ส่วนลด และเป็นฐานให้ระบบคิด Margin เป้าหมาย/ขั้นต่ำ และ Floor Price ให้อัตโนมัติ'],
      ['ใช้ที่ไหน', 'ส่วนลดที่เสนอ = (1 − ราคาที่เสนอ ÷ ราคาขายปกติ) × 100 — ถ้าไม่กรอก ระบบจะเทียบส่วนลดไม่ได้ และขั้นบันไดตามจำนวนจะใช้ไม่ได้'],
    ],
  },
  floor: {
    title: 'Floor Price',
    kind: 'ว่างไว้ = ระบบคิดให้ · กรอกเอง = ใช้ค่าของคุณแทน',
    body: [
      ['คืออะไร', 'ราคาต่ำสุดต่อชิ้นที่ "ห้ามขายต่ำกว่านี้"'],
      ['ถ้าไม่กรอก ระบบคิดให้ยังไง', 'ราคาขายปกติ × (1 − เพดานส่วนลดพิเศษ ÷ 100)\nแต่ไม่ต่ำกว่าราคาเท่าทุนจริง — คือราคาต่ำสุดที่นโยบายส่วนลดยอมให้'],
      ['กรอกเองเมื่อไหร่', 'เมื่ออยากคุมเข้มกว่าสูตร เช่นสินค้าที่ไม่อยากให้ลดลึกแม้สูตรจะยอม — ค่าที่กรอกเองชนะเสมอ'],
  ...
  margins: {
    ...
    kind: 'ว่างไว้ = ระบบคิดให้ · กรอกเอง = ใช้ค่าของคุณแทน · เป็น % ของยอดขาย ไม่ใช่ % ของต้นทุน',
    body: [
      ['คืออะไร', 'เกณฑ์ Margin สองเส้น ใช้ตัดสินว่าราคาที่เซลล์เสนอ "ผ่าน" หรือ "ต่ำกว่าเกณฑ์"'],
      ['ถ้าไม่กรอก ระบบคิดให้ยังไง', 'เป้าหมาย = Margin ที่ได้ตอนขายราคาปกติเต็ม\nขั้นต่ำ = Margin ที่ได้ตอนลดเต็มเพดานส่วนลดพิเศษ\nคิดที่จำนวน 1 ชิ้นเสมอ เพราะเกณฑ์เป็นนโยบาย ไม่ควรขยับตามจำนวนที่ลูกค้าสั่ง'],
      ...
      ['ถ้าจะกรอกเอง', 'กรอกเป็นเปอร์เซ็นต์เต็ม เช่น ต้องการ Margin 25% ให้กรอก 25 ไม่ใช่ 0.25 (กรอก 0.25 ระบบอ่านเป็น 0.25% ซึ่งแทบไม่มีเส้นขั้นต่ำเลย)'],
...
// กรอกเอง = ตัวอักษรปกติ · คิดให้อัตโนมัติ = ตัวเอียงสีจาง จะได้แยกออกว่าตัวไหนคนตั้ง ตัวไหนระบบคิด
function AutoVal({ own, auto, fmt }) {
  const { t } = useLanguage()
  if (own !== null && own !== undefined && own !== '') return <span>{fmt(own)}</span>
  if (auto === null || auto === undefined) return <span style={{ color: 'var(--text-light)' }}>-</span>
  return <span style={{ fontStyle: 'italic', color: 'var(--text-light)' }} title={t('ระบบคิดให้อัตโนมัติ — กรอกเองเพื่อใช้ค่าของคุณแทน')}>{fmt(auto)}</span>
}
                ...
                {filtered.map(r => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td><AutoVal own={c?.floor_price} auto={c?.auto_floor_price} fmt={fmtCurrency} /></td>
                      <td style={{ fontSize: 12, whiteSpace: 'nowrap' }}>
                        <AutoVal own={c?.target_margin_percent} auto={c?.auto_target_margin_percent} fmt={pct} />
                        {' / '}
                        <AutoVal own={c?.minimum_margin_percent} auto={c?.auto_minimum_margin_percent} fmt={pct} />
                      </td>
