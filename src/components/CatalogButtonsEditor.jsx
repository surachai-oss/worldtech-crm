...
function ButtonRow({ row, first, last, onPatch, onDelete, onMove, busy }) {
  const { t } = useLanguage()
  const { toast } = useUi()
  ...
  const pickImage = async (file) => {
    ...
    try {
      ...
      onPatch({ image_url: img.image_url, image_path: img.image_path })
  ...
  return (
    <div style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 10, marginBottom: 8, background: row.is_visible ? '#fff' : '#fafbfc' }}>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
        ...
        <button className="btn btn-outline btn-xs" onClick={() => onPatch({ is_visible: !row.is_visible })}>
      ...
      <div className="form-row">
        <div className="form-group" style={{ marginBottom: 8 }}>
          ...
          <input className="form-control" value={row.label} onChange={e => onPatch({ label: e.target.value })}
            placeholder={t('เช่น ทักแชท LINE')} />
        </div>
        <div className="form-group" style={{ marginBottom: 8 }}>
          <label className="form-label">{t('ชนิด')}</label>
          <select className="form-control" value={row.kind} onChange={e => onPatch({ kind: e.target.value })}>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        <label style={{ fontSize: 11, color: 'var(--text-light)' }}>{t('สีพื้น')}
          <input type="color" value={row.bg_color} onChange={e => onPatch({ bg_color: e.target.value })}
        ...
        <label style={{ fontSize: 11, color: 'var(--text-light)' }}>{t('สีตัวอักษร')}
          <input type="color" value={row.text_color} onChange={e => onPatch({ text_color: e.target.value })}
            style={{ marginLeft: 6, verticalAlign: 'middle', width: 34, height: 24, padding: 0, border: '1px solid var(--border)', borderRadius: 4 }} />
        </label>
        {PRESETS.map(p => (
          <button key={p.name} className="btn btn-outline btn-xs"
            onClick={() => onPatch({ bg_color: p.bg, text_color: p.fg })}>{t(p.name)}</button>
        ))}
      </div>
    </div>
  )
...
export default function CatalogButtonsEditor({ catalogId = null, onChanged }) {
  ...
  const load = async () => {
    setLoading(true)
    try { setRows(await listCatalogButtons(catalogId)) }
  ...
  const announce = (next) => { setRows(next); onChanged?.(next) }

  const add = async () => {
    setBusy(true)
    try {
      const order = rows.length ? Math.max(...rows.map(r => r.display_order)) + 1 : 0
  ...
  // แก้แล้วเขียนลงฐานทันที ไม่มีปุ่มบันทึกแยก — ปุ่มไม่กี่ช่อง การมีปุ่มบันทึกอีกชั้นทำให้ลืมกดมากกว่าช่วย
  const patch = async (row, p) => {
    const next = rows.map(r => (r.id === row.id ? { ...r, ...p } : r))
    announce(next)
    try { await updateCatalogButton(row.id, p) }
    catch (e) { toast('บันทึกไม่สำเร็จ: ' + e.message, 'error'); load() }
  }
    ...
    <div>
      {rows.map((row, i) => (
        <ButtonRow key={row.id} row={row} first={i === 0} last={i === rows.length - 1} busy={busy}
          onPatch={p => patch(row, p)} onDelete={() => remove(row)} onMove={d => move(i, d)} />
      ))}
      {!rows.length && (
