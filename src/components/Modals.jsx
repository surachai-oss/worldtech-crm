...
export function QuotationModal({ initial, companies, defaultCompanyId, isAdmin, onClose, onSave }) {
  const { toast } = useUi()
  const [f, setF] = useState(() => initial || {
    company_id: defaultCompanyId || '', subject: '', status: 'Draft', sale_phone: '',
              ...
              {items.map((it, i) => {
                ...
                return (
                  <tr key={i}>
                    ...
                    <td>
                      <select className="form-control" style={{ marginBottom: 4 }} value={it.product_id} onChange={e => onProductChange(i, e.target.value)} disabled={!products}>
