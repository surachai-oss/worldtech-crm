...
// onCreateNew (ไม่บังคับ): async fn(companyFields) => companyRow — ถ้าใส่มา จะมีปุ่ม "+ ลูกค้าใหม่" ให้เพิ่มลูกค้าใหม่ได้ทันทีโดยไม่ต้องออกจากฟอร์มที่กำลังกรอกอยู่
function CompanySelect({ companies, value, onChange, onCreateNew, isAdmin }) {
  const { toast } = useUi()
  const [showNew, setShowNew] = useState(false)

  const createNew = async (f) => {
    if (!f.name?.trim()) { toast('กรุณากรอกชื่อ', 'error'); return }
    try {
      const company = await onCreateNew(f)
      onChange(company.id)
      setShowNew(false)
    } catch (e) {
      toast('เพิ่มลูกค้าใหม่ไม่สำเร็จ: ' + e.message, 'error')
    }
  }
  ...
  return (
    <div style={{ display: 'flex', gap: 6, alignItems: 'flex-start' }}>
      ...
      {onCreateNew && <button type="button" className="btn btn-outline btn-sm" style={{ whiteSpace: 'nowrap' }} onClick={() => setShowNew(true)}>+ ลูกค้าใหม่</button>}
      {showNew && <CompanyModal isAdmin={isAdmin} hideAttachments onClose={() => setShowNew(false)} onSave={createNew} />}
    </div>
  )
}
...
export function CompanyModal({ initial, isAdmin, hideAttachments, onClose, onSave }) {
...
export function DealModal({ initial, companies, defaultCompanyId, defaultStage, isAdmin, onCreateCompany, onClose, onSave }) {
  const { toast } = useUi()
  const [f, setF] = useState(() => {
  ...
  return (
    <ModalShell title={initial?.id ? 'แก้ไขดีล' : 'เพิ่มดีล'} onClose={onClose} onSave={submit} wide>
      <Field label="บริษัท"><CompanySelect companies={companies} value={f.company_id} onChange={v => setF(s => ({ ...s, company_id: v }))} onCreateNew={onCreateCompany} isAdmin={isAdmin} /></Field>
...
export function QuotationModal({ initial, companies, defaultCompanyId, currentUserName, isAdmin, onCreateCompany, onClose, onSave }) {
  const { toast } = useUi()
  const [f, setF] = useState(() => {
    const base = {
